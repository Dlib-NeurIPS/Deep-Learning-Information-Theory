...
MIFX_vector = np.zeros((num_epochs,3))
MIFY_vector = np.zeros((num_epochs,3))

MIFX_kvector = np.zeros((num_epochs,3))
MIFY_kvector = np.zeros((num_epochs,3))

MIFX_bvector = np.zeros((num_epochs,3))
MIFY_bvector = np.zeros((num_epochs,3))
...
for i in range(num_epochs):
    ...
    for j in range(num_samples):
        ...
        Gibbs_all_f1 = Gibbs_pdf(activation1[j][i])
        Gibbs1 = np.mean(Gibbs_all_f1,0)
        entropy1 = cal_entropy(Gibbs1)
        entropy1_all = cal_entropy(Gibbs_all_f1)

        Gibbs_all_f2 = Gibbs_pdf(activation2[j][i])
        Gibbs2 = np.mean(Gibbs_all_f2,0)
        entropy2 = cal_entropy(Gibbs2)
        entropy2_all = cal_entropy(Gibbs_all_f2)

        Gibbs_all_f3 = Gibbs_pdf(activation3[j][i])
        Gibbs3 = np.mean(Gibbs_all_f3,0)
        entropy3 = cal_entropy(Gibbs3)
        entropy3_all = cal_entropy(Gibbs_all_f3)

        MIX_F1_gibbs = cal_MIFX(Gibbs_all_f1)
        MIX_F2_gibbs = cal_MIFX(Gibbs_all_f2)
        MIX_FY_gibbs = cal_MIFX(Gibbs_all_f3)

        MIY_F1_gibbs = cal_MIFY(Gibbs_all_f1,y_true_batch)
        MIY_F2_gibbs = cal_MIFY(Gibbs_all_f2,y_true_batch)
        MIY_FY_gibbs = cal_MIFY(Gibbs_all_f3,y_true_batch)

        print('The epoch is %d'%(i+1))
        print('MI(F1,X),  MI(F2,X),  and MI(FY,X)  are (%.2f, %.2f, %.2f)'%(MIX_F1_gibbs, MIX_F2_gibbs, MIX_FY_gibbs))
        print('MI(F1,Y),  MI(F2,Y),  and MI(FY,Y)  are (%.2f, %.2f, %.2f)'%(MIY_F1_gibbs, MIY_F2_gibbs, MIY_FY_gibbs))
        print('MI(F1,XO), MI(F2,XO), and MI(FY,XO) are (%.2f, %.2f, %.2f)'%(MIX_F1_gibbs-MIY_F1_gibbs,MIX_F2_gibbs-MIY_F2_gibbs,MIX_FY_gibbs-MIY_FY_gibbs))
        print('')

        MIX_F1_gs += MIX_F1_gibbs
        MIX_F2_gs += MIX_F2_gibbs
        MIX_FY_gs += MIX_FY_gibbs
        MIY_F1_gs += MIY_F1_gibbs
        MIY_F2_gs += MIY_F2_gibbs
        MIY_FY_gs += MIY_FY_gibbs

        MIX_F1_kde,MIY_F1_kde = kde_mi(activation1[j][i],kde_noise_variance,y_true_batch)
        MIX_F2_kde,MIY_F2_kde = kde_mi(activation2[j][i],kde_noise_variance,y_true_batch)
        MIX_FY_kde,MIY_FY_kde = kde_mi(activation3[j][i],kde_noise_variance,y_true_batch)

        MIX_F1_ks += MIX_F1_kde
        MIX_F2_ks += MIX_F2_kde
        MIX_FY_ks += MIX_FY_kde
        MIY_F1_ks += MIY_F1_kde
        MIY_F2_ks += MIY_F2_kde
        MIY_FY_ks += MIY_FY_kde
    
        print('kde: MI(X,F1)=%0.3f, MI(Y,F1)=%0.3f' % (MIX_F1_kde,MIY_F1_kde))
        print('kde: MI(X,F2)=%0.3f, MI(Y,F2)=%0.3f' % (MIX_F2_kde,MIY_F2_kde))
        print('kde: MI(X,FY)=%0.3f, MI(Y,FY)=%0.3f' % (MIX_FY_kde,MIY_FY_kde))
        print('')

        MIX_F1_bin,MIY_F1_bin = bin_mi(activation1[j][i],y_true_batch)
        MIX_F2_bin,MIY_F2_bin = bin_mi(activation2[j][i],y_true_batch)
        MIX_FY_bin,MIY_FY_bin = bin_mi(activation3[j][i],y_true_batch)
        ...
        print('bin: MI(X,F1)=%0.3f, MI(Y,F1)=%0.3f' % (MIX_F1_bin,MIY_F1_bin))
        print('bin: MI(X,F2)=%0.3f, MI(Y,F2)=%0.3f' % (MIX_F2_bin,MIY_F2_bin))
        print('bin: MI(X,FY)=%0.3f, MI(Y,FY)=%0.3f' % (MIX_FY_bin,MIY_FY_bin))
        print('')


    MIFX_vector[i] = [MIX_F1_gs/num_samples, MIX_F2_gs/num_samples, MIX_FY_gs/num_samples]
    MIFY_vector[i] = [MIY_F1_gs/num_samples, MIY_F2_gs/num_samples, MIY_FY_gs/num_samples]
    ...
    MIFX_bvector[i] = [MIX_F1_es/num_samples, MIX_F2_es/num_samples, MIX_FY_es/num_samples]
    MIFY_bvector[i] = [MIY_F1_es/num_samples, MIY_F2_es/num_samples, MIY_FY_es/num_samples]

np.savez('MLP_IT_ReLU.npz',train_acc=np.mean(acc_vector,axis=0),kld=np.mean(kld_vector,axis=0),MIFX_gibbs=MIFX_vector,MIFY_gibbs=MIFY_vector,MIFX_kde=MIFX_kvector, MIFY_kde=MIFY_kvector,MIFX_bin=MIFX_bvector,MIFY_bin=MIFY_bvector)
