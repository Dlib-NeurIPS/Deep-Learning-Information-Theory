...
with g.as_default():
    ...
    #loss = tf.reduce_mean(tf.keras.losses.MSE(y_true=tf_y, y_pred=y_pred), name='cost')
...
MIFX_vector = np.zeros((n_samples,training_epochs,3))
#MIY_vector contains the mutual information between Y and each layer
MIFY_vector = np.zeros((n_samples,training_epochs,3))
#MIXO_vector contains the mutual information between X_bar and each layer
MIFXO_vector = np.zeros((n_samples,training_epochs,3))
...
with tf.Session(graph=g) as sess:
    for k in range(n_samples):
        ...
        for epoch in range(training_epochs):
            ...
            activation1, activation2, activation3 = sess.run([fc1, fc2, output_logits], feed_dict={'features:0': train_samples,
            ...
            MIFX_vector[k,epoch] = [cal_MIFX(Gibbs_all_f1), cal_MIFX(Gibbs_all_f2), cal_MIFX(Gibbs_all_f3)]
            MIFY_vector[k,epoch] = [cal_MIFY(Gibbs_all_f1,train_labels_onehot), cal_MIFY(Gibbs_all_f2,train_labels_onehot), cal_MIFY(Gibbs_all_f3,train_labels_onehot)]
            MIFXO_vector[k,epoch] = [MIFX_vector[k,epoch,0]-MIFY_vector[k,epoch,0],MIFX_vector[k,epoch,1]-MIFY_vector[k,epoch,1],MIFX_vector[k,epoch,2]-MIFY_vector[k,epoch,2]]
            kld_vector[k,epoch] = avg_cost/(i + 1)
            kld_vector2[k,epoch] = test_loss

            print('MI(F1,X),  MI(F2,X),  and MI(FY,X)  are (%.2f, %.2f, %.2f)'%(MIFX_vector[k,epoch,0],MIFX_vector[k,epoch,1],MIFX_vector[k,epoch,2]))
            print('MI(F1,Y),  MI(F2,Y),  and MI(FY,Y)  are (%.2f, %.2f, %.2f)'%(MIFY_vector[k,epoch,0],MIFY_vector[k,epoch,1],MIFY_vector[k,epoch,2]))
            print('MI(F1,XO), MI(F2,XO), and MI(FY,XO) are (%.2f, %.2f, %.2f)'%(MIFXO_vector[k,epoch,0],MIFXO_vector[k,epoch,1],MIFXO_vector[k,epoch,2]))       
            print("Samples: %02d Epoch: %03d | AvgCost: %.3f/%.3f Train/Valid ACC: %.3f/%.3f" % (k+1,epoch + 1, avg_cost/(i + 1),test_loss,train_acc, test_acc))

        if activation_func == 'relu':
            np.savez('MLP_IT_MNIST_%03d_%03d_relu.npz'%(n_hidden_1,n_hidden_2),train_acc=train_acc_vector,test_acc=test_acc_vector,kld=kld_vector,kld2=kld_vector2,MIFX=MIFX_vector,MIFY=MIFY_vector,MIFXO=MIFXO_vector)
        elif activation_func == 'sigmoid':
            np.savez('MLP_IT_MNIST_%03d_%03d_sigmoid.npz'%(n_hidden_1,n_hidden_2),train_acc=train_acc_vector,test_acc=test_acc_vector,kld=kld_vector, MIFX=MIFX_vector,MIFY=MIFY_vector,MIFXO=MIFXO_vector)
        else:
            np.savez('MLP_IT_MNIST_%03d_%03d_tanh.npz'%(n_hidden_1,n_hidden_2),train_acc=train_acc_vector,test_acc=test_acc_vector,kld=kld_vector, MIFX=MIFX_vector,MIFY=MIFY_vector,MIFXO=MIFXO_vector)
